import { Observable, combineLatest, forkJoin, interval, timer } from "rxjs";
...
(() => {
  ...
  //* Solution with combineLatest(object)
  // combineLatest({ let: letras$, num: numeros$ })
  //\   .pipe(map((val) => `${val.let}${val.num}`))
  //\   .subscribe(console.log);

  //* Solution with combineLatest(array)
