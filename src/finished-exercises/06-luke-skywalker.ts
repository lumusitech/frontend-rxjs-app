...
import { zip, of, Observable, forkJoin } from "rxjs";
...
(() => {
  ...
  //         map((species) => ({ species, character }))
  //       )
  //     )
  //   )

  //* Solution 2 (no resuelve como pide el ejercicio):
  // forkJoin({
  //   character: getRequest(`${SW_API}/people/1`),
  //   species: getRequest(`${SW_API}/species/1`),
  // })

  //* Solution 3: belong to Fernando Herrera:
  getRequest(`${SW_API}/people/1`)
    .pipe(
      // Solution basic - first challenge
      // switchMap((resp) => getRequest(resp.starships[0]))
      //| zip function emits an array with the values of the observables given in an array
      switchMap((resp) => zip(of(resp), getRequest(resp.starships[0]))),
      //| map converts array to object
